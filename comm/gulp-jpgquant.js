...
// consts
const PLUGIN_NAME = 'gulp-imgquant';
...
function gulpPngquant(options, customPngquantPath) {
    ...
    var stream = through.obj(function(file, enc, cb) {
        ...
        if (file.isBuffer()) {
            ...
            if (path.extname(file.relative) === '.png') {
              console.log(chalk.blue('pngquant compressing: ') + file.relative);

              file.contents = spawnSync(customPngquantPath || pngquant, opts, {
                  input: file.contents
              }).stdout;
            } else if (path.extname(file.relative) === '.jpg') {
              file.contents = spawnSync(mozjpeg, opts, {input: file.contents}).stdout;
            }
        }
